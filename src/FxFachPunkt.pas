...
begin
  ...
  begin
    ...
    begin
      FFachPunktNode.ChildValues['Text'] := lblFachPunkt.Caption;
...
var
  FontNode: IXMLNode;
begin
  FFachPunktNode := aXMLNode;

  if Assigned(FFachPunktNode) and FFachPunktNode.HasChildNodes then
  begin
    FachPunktText := CheckLexicon(VarToStr(FFachPunktNode.ChildValues['Text']));

    OrderNr := StrToIntDef(VarToStr(FFachPunktNode.ChildValues['SortNr']), 0);

    if Assigned(FFachPunktNode.ChildNodes.FindNode('Farbe')) then
      if StrToIntDef(VarToStr(FFachPunktNode.ChildValues['Farbe']), 0) > 0 then
        FachPunktColor := clGray;

    FontNode := FFachPunktNode.ChildNodes.FindNode('Font');
    ...
    Wert := StrToIntDef(VarToStr(FFachPunktNode.ChildValues['Wert']), 0);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      FWert := aWert;
      FFachPunktNode.ChildValues['Wert'] := Wert;
...
procedure TfrmFachPunkt.SaveFachPunktNode;
begin
  if Assigned(FFachPunktNode) then
  begin
    FFachPunktNode.ChildValues['Text'] := FachPunktText;
    FFachPunktNode.ChildValues['Wert'] := Wert;
  end;
end;
