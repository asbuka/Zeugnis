...
type
  ...
  TFachPunktChangeEvent = procedure(Sender: TfrmTextFach; FachPunkt, Wert: Word) of object;
  ...
  TfrmTextFach = class(TForm)
    ...
    procedure reTextFachMouseDown(Sender: TObject; Button: TMouseButton;
  ...
  private
    ...
    FOnFachPunktChange: TFachPunktChangeEvent;
...
//    procedure SetRichEditAlign(const Value: TRichEditAlign);
  ...
  public
    ...
    property OnFachPunktChange: TFachPunktChangeEvent read FOnFachPunktChange write FOnFachPunktChange;
...
begin
  ...
  begin
    ...
    begin
      if SameText('TORTE' + FormatFloat('00', TortenNr), VarToStr(FPlatzHalter.ChildNodes.Get(Idx).Attributes['NAME'])) then
      begin
...
procedure TfrmTextFach.InitFromXML(aXMLNode, aXMLGGLNode: IXMLNode; AlignBlocksatz: Boolean);
var
  RTFTextNode: IXMLNode;
  WorkStream: TStringStream;
begin
  FTextFachNode    := aXMLNode;
  FTextFachGGLNode := aXMLGGLNode;
  ...
  begin
    ...
    begin
      WorkStream := TStringStream.Create;
      WorkStream.WriteString(VarToStr(RTFTextNode.NodeValue));
      WorkStream.Position := soFromBeginning;
      reTextFach.Lines.LoadFromStream(WorkStream);
      WorkStream.Free;

//      reTextFach.SelectAll;
////      reTextFach.Paragraph.LeftIndent  := 25;
//      reTextFach.Paragraph.FirstIndent := 25;
//      reTextFach.Paragraph.RightIndent := 25;

      if AlignBlocksatz then
        RichEditAlignBlocksatz(reTextFach);
      ...
      if Assigned(FTextFachNode.ChildNodes.FindNode('Schriftgrad')) then
        if not VarIsNull(FTextFachNode.ChildValues['Schriftgrad']) then
          SetSchriftgrad(StrToIntDef(VarToStr(FTextFachNode.ChildValues['Schriftgrad']), MemoSGDef));

      FPlatzHalter := FTextFachNode.ChildNodes.FindNode('PLATZHALTER');
      if not IsRTF(VarToStr(RTFTextNode.NodeValue)) then
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        TorteNr := StrToIntDef(NurZiffern(GetEffektText(reTextFach, WStart + WLen + 1, CFM_STRIKEOUT)), 0);
        if TorteNr > 0 then
        begin
...
begin
  ...
  try
    ...
    begin
      ...
      PHStart := reTextFach.FindText(PHName, PHStart, reTextFach.GetTextLen, [stWholeWord]);
      if PHStart >= 0 then
      begin
        reTextFach.SelStart            := PHStart;
        reTextFach.SelLength           := Length(PHName);
        reTextFach.SelAttributes.Color := clRed;
        reTextFach.SelAttributes.Style := reTextFach.SelAttributes.Style + [fsUnderline];

        reTextFach.SelStart            := PHStart + Length(PHName);
        reTextFach.SelLength           := 0;
        reTextFach.SelText             := ' ' + FormatFloat('00', StrToInt(NurZiffern(PHName)));

        reTextFach.SelStart            := PHStart + Length(PHName);
        reTextFach.SelLength           := 3;
        reTextFach.SelAttributes.Size  := 1;
        reTextFach.SelAttributes.Style := reTextFach.SelAttributes.Style - [fsUnderline] + [fsStrikeOut];
        reTextFach.SelAttributes.Color := clWhite;
      end;
    end;
  finally
    reTextFach.Lines.EndUpdate;
...
begin
  ...
  begin
    Wert := 0;
    FachGGLPunkt := GetPunktGGLFach(PHTortenMenu.Tag);
    if Assigned(FachGGLPunkt) then
    begin
      ...
      FachGGLPunkt.ChildValues['Wert'] := Wert;
    end;

    if Assigned(FOnFachPunktChange) then
      OnFachPunktChange(Self, PHTortenMenu.Tag, Wert);
  end;
end;
...
procedure TfrmTextFach.reTextFachMouseDown(Sender: TObject;
...
begin
  ...
  begin
    APoint := Point(X, Y);
    cPos :=  SendMessage(TRichEdit(Sender).Handle, EM_CHARFROMPOS, 0, Integer(@APoint));
    if (cPos > 0) and (cPos < TRichEdit(Sender).GetTextLen) then
    begin
      TextFachMapItem := FTextFachMap.GetTextFachMapItemFromPos(cPos);
      if Assigned(TextFachMapItem) and (TextFachMapItem.TorteNr <> 0) then
      begin
        Label1.Caption := 'Torte Nr.: ' + IntToStr(TextFachMapItem.TorteNr);
        TortenValueNode := FindTortenValue(TextFachMapItem.TorteNr);
        if Assigned(TortenValueNode) then
        begin
          SendMessage(TRichEdit(Sender).Handle, EM_SETSEL, WPARAM(TextFachMapItem.WStart), LPARAM(TextFachMapItem.WStart + TextFachMapItem.WLen));
          OldWstart := TextFachMapItem.WStart;
          OldWLen := TextFachMapItem.WLen;
          Label2.Caption := 'Sel. Text: ' + TRichEdit(Sender).SelText;
          Label3.Caption := 'Sel. Start: ' + IntToStr(OldWstart) + ' Len: ' + IntToStr(OldWLen);
          BuildPopupMenu(TortenValueNode, TextFachMapItem.TorteNr);
          TRichEdit(Sender).PopupMenu := PHTortenMenu;
        end else
        begin
          PHTortenMenuClear;
          TRichEdit(Sender).PopupMenu := nil;
        end;
      end else
      begin
        PHTortenMenuClear;
...
begin
  if cmbSchriftgrad.ItemIndex <> cmbSchriftgrad.Items.IndexOf(IntToStr(Value)) then
  begin
