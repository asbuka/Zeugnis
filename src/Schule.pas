...
uses
  ...
  System.IniFiles, System.Win.Registry, System.SysUtils, System.SyncObjs, Vcl.ExtActns,
...
type
  ...
  function WMI_Get_Betriebssystem(const MitVersion: Boolean = False): string;
  ...
  function IsHTML(sText: string): Boolean;
  function RTF2PlainText(sRTF: String): string;
  function RTFOhneFarbe(sRTF: String; sFachName: string = ''): string;
...
function RTFOhneEffectText(sRTF: String; aMask: Cardinal): string;
var
  aRE: TRichEdit;
...
//  sRTF := StringReplace(sRTF,'\tab','\tab ',[rfReplaceAll]);
  ...
  try
    ...
    while Pos <= Cardinal(aRE.GetTextLen) do
    begin
      if GetEffektText(aRE, Pos, aMask, WStart, WLen) then
      begin
        aRE.SelStart  := WStart;
        aRE.SelLength := WLen;
        aRE.SelText   := '';
...
var
  ...
  aStream: TStringStream;
begin
  TmpRTF := RTFOhneEffectText(sRTF, CFM_STRIKEOUT);

  aRE := TRichEdit.Create(nil);
  //RTF Bug: Beim Laden des Texttes über
  //LoadFromStream werden die ersten Zeichen nach dem Tab verschluckt.
  // Deswegen hier dieser Wokarround
//  TmpRTF := StringReplace(TmpRTF,'\tab','\tab ',[rfReplaceAll]);
  aStream := TStringStream.Create(TmpRTF);
  try
    System.TMonitor.Enter(CS);
    aRE.Visible := False;
    aRE.ParentWindow := Application.Handle;
...
function WMI_Get_Betriebssystem(const MitVersion: Boolean): string;
const
  wbemFlagForwardOnly = $00000020;
  WMIQuery = 'SELECT Name, Version, BuildNumber FROM Win32_OperatingSystem';
  WQL = 'WQL';
  Localhost = 'localhost';
...
var
  ...
  oEnum: IEnumvariant;
  iValue: LongWord;
  posMicrosoft, posDelimiter: Integer;
begin
  Result := '?';
  try
    FSWbemLocator := CreateOleObject('WbemScripting.SWbemLocator');
    FWMIService := FSWbemLocator.ConnectServer(Localhost, RootCIMV2, '', '');
    FWbemObjectSet := FWMIService.ExecQuery(WMIQuery, WQL, wbemFlagForwardOnly);
    oEnum := IUnknown(FWbemObjectSet._NewEnum) as IEnumVariant;
    if oEnum.Next(1, FWbemObject, iValue) = 0 then
    begin
      if not VarIsNull(FWbemObject.Name) and not VarIsEmpty(FWbemObject.Name) then
      begin
        Result := string(FWbemObject.Name);
        posDelimiter := Pos('|', Result);
        if posDelimiter > 0 then
          Result := Copy(Result, 1, posDelimiter - 1);

        posMicrosoft := Pos(MicrosoftPrefix, Result);
        if posMicrosoft > 0 then
          Result := Trim(Copy(Result, posMicrosoft + Length(MicrosoftPrefix), 40));

        if MitVersion and not VarIsNull(FWbemObject.Version) and not VarIsEmpty(FWbemObject.Version) then
    ...
    on E: Exception do
      Result := '?';  // Log or handle the exception as needed
  end;
end;
...
begin
  sText := Trim(sText);
  Result := Pos(Copy(sText, 1, 100), '<html>') >= 0;
end;
...
begin
  if Succeeded(SHGetSpecialFolderLocation(Form.Handle, CSIDL_PERSONAL, PI)) then
  try
    if SHGetPathFromIDList(PI, Path) then
      Result := Path
    else
      Result := '';
  finally
...
begin
  ...
  begin
    ...
    begin
      ...
      SelStart := Idx;
      Idx := 1;
      while SelStart + Idx <= aRichEdit.GetTextLen do //subtract the & from line
      begin
...
begin
  ...
  if numPapers > 0 then
    WinApi.WinSpool.DeviceCapabilities(Device, Port, DC_PAPERSIZE, PChar(@sizes[0]), nil);
end;
