...
type
  ...
  function WMI_Get_Betriebssystem(const MitVersion: Boolean = False): ShortString;
...
function WMI_Get_Betriebssystem(const MitVersion: Boolean): ShortString;
const
...
begin
  ...
  try
    ...
    FWbemObjectSet := FWMIService.ExecQuery('SELECT Name, Version, BuildNumber FROM Win32_OperatingSystem', 'WQL', wbemFlagForwardOnly);
    oEnum := IUnknown(FWbemObjectSet._NewEnum) as IEnumVariant;
    if oEnum.Next(1, FWbemObject, iValue) = 0 then
    begin
      Result := ShortString(FWbemObject.Name);
      if Pos(ShortString('|'), Result) > 0 then
        Result := Copy(Result, 1, Pos(ShortString('|'), Result) - 1);

      if Pos(ShortString('Microsoft '), Result) > 0 then
        Result := Trim(Copy(Result, Pos(ShortString('Microsoft '), Result) + 10, 40));

      if MitVersion then
        Result := Result + ' [' + ShortString(FWbemObject.Version) + ']';
